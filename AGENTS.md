...
**Grove** is a **local-first, markdown-based operating system for managing one person's life and work** from a single folder. Every note is a typed object. Everything is plain markdown — portable forever, owned by the user, no vendor lock-in. **Version:** see `VERSION` (`v26.05.30-01`); updates are CORE-only and backward-compatible (`99 System/_system/core-manifest.md`).
