# Personal OS — Claude Code entry

You are operating a **Personal OS** (a local-first life/work system). Your full, model-agnostic contract is **`AGENTS.md`** — **read it now and follow it** before doing anything else. (`AGENTS.md` is canonical; this file only bootstraps Claude Code.)
...
The forceful rule above is scoped to the **un-onboarded** state only — it overrides the global instinct to "lead with the answer" *just for a fresh vault*. Once onboarded, normal behaviour resumes.
